...
-(void)pushViewControllerWithLRAnimated:(UIViewController *)viewController{
    ...
    //绘图
    //创建上下文（大小、是否透明-YES为黑色、缩放因子-0则根据屏幕大小变化）
    ...
    UIImage * img = UIGraphicsGetImageFromCurrentImageContext();//从当前上下文获取一个uiimage对象
...
- (void)moveViewWithX:(float)x
{
    ...
    NSLog(@"--%@",NSStringFromCGRect(lastScreenShotView.frame));
}
