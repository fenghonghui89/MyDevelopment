...
//                inset.bottom = self.frame.origin.y - _scrollView.contentSize.height +kViewHeight;
                inset.bottom = self.frame.origin.y - _scrollView.contentSize.height + self.frame.size.height;
                _scrollView.contentInset = inset;
                
                // 2.设置滚动位置
//                _scrollView.contentOffset = CGPointMake(0, self.validY + kViewHeight);
                 _scrollView.contentOffset = CGPointMake(0, self.validY + self.frame.size.height);
            }];
			break;
        }
	}
...
//    if (refreshFootMoreView.canLoadMore &&refreshFootMoreView.isDragging) {
        if (scrollViewSystem.contentOffset.y + scrollViewSystem.frame.size.height >= scrollViewSystem.contentSize.height) {
//            [[refreshFootMoreView infoLabel] setText:@"加载中..."];
//            [scrollView setScrollEnabled:YES];
//            [refreshFootMoreView setIsDragging:NO];
//            [self requestThemeDateNextPage];
...
- (CGFloat)validY
{
//    NSLog(@"%f",MAX(_scrollView.contentSize.height, _scrollView.frame.size.height) - _scrollView.frame.size.height-40);
