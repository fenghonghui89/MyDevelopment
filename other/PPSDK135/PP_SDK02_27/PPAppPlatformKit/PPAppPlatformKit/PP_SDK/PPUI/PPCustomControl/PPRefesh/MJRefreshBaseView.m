...
- (void)initial
{
    ...
    arrowImage.image = [PPUIKit setLocaImage:@"arrow"];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([@"contentOffset" isEqualToString:keyPath]) {
        ...
        if (_scrollView.isDragging) {
            //为了适配横屏时不需要下拉这么多偏移量，所以注释这句
//            CGFloat validOffsetY = validY + kViewHeight;
            //为了适配通知中心tableview的contenInset初始时不为0所以注释这句
//            CGFloat validOffsetY = validY + self.frame.size.height;
            CGFloat validOffsetY = validY + self.frame.size.height + self.contentInsetTop;
