...
/*
 ...
 mutating 类不能用这个关键字 因为不像struct/enum有变异方法 也没必要有变异方法
 ...
 class 修饰类属性 必须指定值，因为无法在构造过程中设置
                  不能用闭包/等号赋默认值 只能用get方法设置默认值 不能写set否则死循环
                  子类可以复写 复写依旧只能用get方法设置默认值 不能写set否则死循环
        修饰类方法 子类可以复写
 
 static 修饰类属性 必须指定值，因为无法在构造过程中设置
                  可以用闭包/等号/get方法 赋默认值
                  子类不能复写
        修饰类方法 子类不能复写
 
 willSet/didSet 作用类似oc的set
                初始化的时候不会触发
                哪怕新值和旧值一样 也会触发
                willSet有newValue，didset有oldValu
                如果在didSet监视器里为属性赋值，这个值会替换监视器之前设置的值
 
 subscript 附属脚本 调用方法为 objName[parama]，例如 person[10]
                  必须要有get，set可选

 
 deinit 反构造 相当于dealloc 子类不能复写 不用加()
 convenience 便利构造方法 一般为 convenience init(参数可有可无){self.init...}
 
 存储属性  实例的 枚举、结构体类型可以没有默认值
          类的  所有类型要有默认值
