...
#import "AAPLTestViewController.h"
...
@interface AAPLTestViewController () <PHPhotoLibraryChangeObserver>
...
@implementation AAPLTestViewController
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  CGSize cellSize = ((UICollectionViewFlowLayout *)self.cv.collectionViewLayout).itemSize;
  AssetGridThumbnailSize = CGSizeMake(cellSize.width * scale, cellSize.height * scale);
  NSLog(@"cellsize %@",NSStringFromCGSize(CGSizeMake(cellSize.width * scale, cellSize.height * scale)));
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  NSLog(@"cv:%@",NSStringFromCGRect(self.cv.frame));
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UICollectionView *collectionView = self.cv;
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
  ...
  NSLog(@"photo count %ld",(long)count);
  return count;
}
...
- (void)updateCachedAssets {
  ...
  NSLog(@"cv %@",NSStringFromCGRect(self.cv.bounds));//随拖动y会变，下拉为-，scrollview滚动原理
  preheatRect = CGRectInset(preheatRect, 0.0f, -0.5f * CGRectGetHeight(preheatRect));
  NSLog(@"preheatRect %@",NSStringFromCGRect(preheatRect));
  //高度向上下各延伸原来高度的一半,y随拖动会变
...
//  NSLog(@"midY %f %f",CGRectGetMidY(preheatRect),CGRectGetMidY(self.previousPreheatRect));
//  NSLog(@"delta:%f %@",delta,NSStringFromCGRect(self.previousPreheatRect));
  if (delta > CGRectGetHeight(self.cv.bounds) / 3.0f) {
...
//      NSLog(@"moveRect %@",NSStringFromCGRect(removedRect));
      NSArray *indexPaths = [self.cv aapl_indexPathsForElementsInRect:removedRect];
      [removedIndexPaths addObjectsFromArray:indexPaths];
    } addedHandler:^(CGRect addedRect) {
//      NSLog(@"addedRect %@",NSStringFromCGRect(addedRect));
      NSArray *indexPaths = [self.cv aapl_indexPathsForElementsInRect:addedRect];
    ...
    NSLog(@"self.previousPreheatRect %@",NSStringFromCGRect(self.previousPreheatRect));
  }
}
