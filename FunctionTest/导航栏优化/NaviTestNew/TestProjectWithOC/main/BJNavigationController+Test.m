...
+ (void)load{
    ...
    [BJNavigationController swizzlingMethodWithOriginal:originalSEL swizzled:swizzledSEL];
    ...
    [BJNavigationController swizzlingMethodWithOriginal:popOriginalSEL swizzled:popSwizzledSEL];
...
- (void)dealNavBarChangeAction:(id<UIViewControllerTransitionCoordinatorContext>)context {
    if ([context isCancelled]) {// 取消了(还在当前页面)
        ...
        BJBaseViewController *fromVC = [context viewControllerForKey:UITransitionContextFromViewControllerKey];
        CGFloat fromVCAlpha = fromVC.xa_navBarAlpha;
        ...
        BJBaseViewController *toVC = [context viewControllerForKey:UITransitionContextToViewControllerKey];
        CGFloat toVCAlpha = toVC.xa_navBarAlpha;
        [UIView animateWithDuration:animdDuration animations:^{
