...
- (void)awakeFromNib {
  ...
  //  NSInteger count = [smartAlbums countOfAssetsWithMediaType:PHAssetMediaTypeImage];
  ...
  //      PHFetchResult *assetresule = [PHAsset fetchAssetsInAssetCollection:assetCollection options:allPhotosOptions];
  //      for (int i = 0; i < assetresule.count; i++) {
  //        PHAsset *asset = assetresule[i];
  //        [self.sectionFetchResults addObject:asset];
  //      }
  //    }else {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
   ...
   */
  if (![segue.destinationViewController isKindOfClass:[AAPLTestViewController class]] || ![sender isKindOfClass:[UITableViewCell class]]) {
    return;
  }
  
  AAPLTestViewController *assetGridViewController = segue.destinationViewController;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath.section == 0) {
    ...
    PHFetchResult *result = self.sectionFetchResults[indexPath.section];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%lu",(unsigned long)result.count];
  } else {
    PHFetchResult *fetchResult = self.sectionFetchResults[indexPath.section];
    PHCollection *collection = fetchResult[indexPath.row];
    PHAssetCollection *assetCollection = (PHAssetCollection *)collection;
