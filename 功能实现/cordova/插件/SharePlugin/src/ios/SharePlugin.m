...
- (void)share:(CDVInvokedUrlCommand*)command
{
  ...
  pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"share OK"];
...
- (void)login:(CDVInvokedUrlCommand*)command
{
  ...
  NSString *shareUrl = [command.arguments objectAtIndex:3];
  NSLog(@"%@ %@ %@ %@",title,content,imgUrl,shareUrl);
  pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"login OK"];
