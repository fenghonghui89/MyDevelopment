...
dispatch_queue_t
dispatch_get_global_queue(long identifier, unsigned long flags);//获取全局线程 参数：1.线程优先级 2.预留参数（一般没用，直接给0） 系统给每一个应用程序提供了三个concurrent dispatch queues。这三个并发调度队列是全局的，它们只有优先级的不同。
...
DISPATCH_NOTHROW
dispatch_queue_t
dispatch_queue_create(const char *label, dispatch_queue_attr_t attr);//创建自定义线程 1、标识符（推荐反向域名） 2、是串行队列DISPATCH_QUEUE_SERIAL（线程同步）还是并行队列DISPATCH_QUEUE_CONCURRENT（线程异步），低版本系统要传NULL
