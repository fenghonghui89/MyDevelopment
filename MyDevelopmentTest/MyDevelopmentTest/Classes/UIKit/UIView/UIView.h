...
typedef NS_ENUM(NSInteger, UIViewAnimationCurve) {
  UIViewAnimationCurveEaseInOut,         // 开始和结束时速度减慢
  UIViewAnimationCurveEaseIn,            // 开始时速度缓慢
  UIViewAnimationCurveEaseOut,           // 结束时速度缓慢
  UIViewAnimationCurveLinear             // 速度不变
};
...
@property(nonatomic) CGFloat           contentScaleFactor NS_AVAILABLE_IOS(4_0);//缩放因子，高清屏为2.0，以前为1.0

@property(nonatomic,getter=isMultipleTouchEnabled) BOOL multipleTouchEnabled;   // 是否支持多点触控default is NO
@property(nonatomic,getter=isExclusiveTouch) BOOL       exclusiveTouch;         // 是否独占整个touch事件，默认为no
...
@property(nonatomic) BOOL               autoresizesSubviews; // default is YES. if set, subviews are adjusted according to their autoresizingMask if self.bounds changes子视图是否自动调整大小，默认为yes
@property(nonatomic) UIViewAutoresizing autoresizingMask;    //自动调整大小选项 simple resize. default is UIViewAutoresizingNone
...
- (BOOL)isDescendantOfView:(UIView *)view;  // returns YES for self.判断view是不是指定视图的子视图
