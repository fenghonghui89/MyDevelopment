...
/* The unselected image is autogenerated from the image argument. The selected image
 ...
 只有设置了image，selectedImage才有效
 如果只设置了image，selectedImage会由系统根据image给出
 默认图片受系统渲染影响，颜色不会显示原来的颜色，如果要显示原来的颜色，img都要设置为UIImageRenderingModeAlwaysOriginal
 */
...
@property(nullable, nonatomic, copy) NSString *badgeValue;    // default is nil 右上角的气泡文字
...
/* To set item label text attributes use the appearance selectors available on the superclass, UIBarItem.
 ...
 如果用了父类的text attributes 可以用titlePositionAdjustment修正可视中心
 */
