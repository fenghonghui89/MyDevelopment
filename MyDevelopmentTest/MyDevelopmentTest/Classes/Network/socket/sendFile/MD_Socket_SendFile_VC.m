...
- (IBAction)sendFileBtnTap:(id)sender {
  ...
  NSString *filePath = @"/Users/hanyfeng/Desktop/知识经验 - socket.rtf";
  NSData *fileData = [NSData dataWithContentsOfFile:filePath];
  NSString *fileName = [filePath lastPathComponent];
  NSInteger fileLength = fileData.length;
  NSString *headerStr = [NSString stringWithFormat:@"file&&%@&&length&&%ld",fileName,fileLength];
...
-(void)onSocket:(AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
  ...
  if ([type isEqualToString:@"file"]) {
    ...
    NSString *fileLength = arr[3];
    
    NSData *fileData = [data subdataWithRange:NSMakeRange(100, data.length-100)];
  }else{
