...
NS_INLINE CF_RETURNS_RETAINED CFTypeRef CFBridgingRetain(id X) {使用CFBridgingRetain。//则需要自己手动释放（CFRelease）f类型转为cf类型
    return (__bridge_retained CFTypeRef)X;
}

NS_INLINE id CFBridgingRelease(CFTypeRef CF_CONSUMED X) { //减少引用计数 cf类型转为f类型，不需要管理释放
