...
- (instancetype)initForWritingWithMutableData:(NSMutableData *)data;//以参数Data为源创建一个归档对象

@property (nullable, assign) id <NSKeyedArchiverDelegate> delegate;

@property NSPropertyListFormat outputFormat;

- (void)finishEncoding;//完成编码
...
- (void)encodeObject:(nullable id)objv forKey:(NSString *)key;//把一个对象与键值关联起来并进行编码
...
- (instancetype)initForReadingWithData:(NSData *)data;//以参数data为源创建一个反归档对象
...
- (nullable id)decodeObjectForKey:(NSString *)key;//按照键值解码反归档对象，得到一个对象并返回
