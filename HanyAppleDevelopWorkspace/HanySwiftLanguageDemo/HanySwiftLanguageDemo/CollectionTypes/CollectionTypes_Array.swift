...
private func func1_4() {
    let fourInts = [Int](repeating: 1, count: 4);
    let fourStrs = [String](repeating: "1", count: 2);
    print(fourInts,fourStrs);
}

//MARK:数组区间赋值
private func func1_6() {
    var pros = [1,1,1];
    pros[0...2] = [2,2,2];//222
    ...
    pros[0...4] = [7,7,7];//777 赋值元素个数小于区间 则删除区间"内"的多余原始元素 区间外不变
    pros[0...1] = [8];//87 赋值元素个数小于区间 则删除区间"内"的多余原始元素 区间外不变
    print(pros);
    
}

//MARK:添加 插入 删除 同类型数组相加(相当于前后连接 数组类型必须相同)
private func func1_7() {
    
    var citys = ["北京","上海","杭州","深圳"];
    
    citys.append("广州");
    
    citys.insert("西安", at: 2);
    
    citys.removeFirst();
    citys.removeLast();
    citys.remove(at: 1);
    citys.removeAll(keepingCapacity: true);
...
private func func1_8() {
    ...
    for (index,value) in provices.enumerated(){
        print("\(index): \(value)")
    }
}

//MARK:swift的array是结构体形式实现的，是值类型
private func func1_9()  {
